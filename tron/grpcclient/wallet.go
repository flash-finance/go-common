...
//GetAssetIssueByID 根据账户地址查询账户发型的通证信息
func (w *Wallet) GetAssetIssueByID(id string) (*core.AssetIssueContract, error) {
	ctx, cancel := getTimeoutContext(defaultTimeout)
	defer cancel()
	callOpt := getDefaultCallOptions()
	...
	msg.Value = []byte(id)
	assetIssue, err := w.client.GetAssetIssueById(ctx, msg, callOpt)

	if nil == assetIssue {
		return nil, err
	}
	return assetIssue, nil
}
